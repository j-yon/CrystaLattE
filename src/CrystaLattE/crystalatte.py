...
def input_parser(in_f_name):
    ...
    if "makefp" in keywords["cle_run_type"]:
        if len(keywords["cle_run_type"]) > 1:
            if (
                "timings" not in keywords["cle_run_type"]
            ):  # TODO: What if there are more than two methods?!
                print(
        ...
        if len(keywords["cle_run_type"]) < 2:
            ...
            for kw_key in kw_keys:
                if type(keywords[kw_key]) == str:
                    func_str = func_str + "{}='{}', ".format(
                        kw_key, str(keywords[kw_key])
                    )
...
def read_cif(fNameIn):
    ...
    try:
        ...
        print(
            "\nERROR: A type error occured when opening or reading CIF file '{0}'".format(
                fNameIn
            )
        )
        sys.exit()
        ...
        print(
            "\nERROR: An unknown error occured when opening or reading CIF file '{0}'".format(
                fNameIn
            )
        )
        sys.exit()
    ...
    try:
        ...
        data["_cell_length_a"] = float(
            data_block["_cell_length_a"].replace("(", "").replace(")", "")
        )
        data["_cell_length_b"] = float(
            data_block["_cell_length_b"].replace("(", "").replace(")", "")
        )
        data["_cell_length_c"] = float(
            data_block["_cell_length_c"].replace("(", "").replace(")", "")
        )
        data["_cell_angle_alpha"] = float(
            data_block["_cell_angle_alpha"].replace("(", "").replace(")", "")
        )
        data["_cell_angle_beta"] = float(
            data_block["_cell_angle_beta"].replace("(", "").replace(")", "")
        )
        data["_cell_angle_gamma"] = float(
            data_block["_cell_angle_gamma"].replace("(", "").replace(")", "")
        )
        data["_cell_volume"] = float(
            data_block["_cell_volume"].replace("(", "").replace(")", "")
        )
...
# ======================================================================
def cif_driver(
    cif_input, cif_output, cif_a, cif_b, cif_c, monomer_cutoff, nmer_cutoff, verbose=1
):
...
def cif_main(fNameIn, fNameOut, Na, Nb, Nc, monomer_cutoff, nmer_cutoff, make_rect_box):
    ...
    if abs(V - volume) > 0.1:
        print("{}".format("~" * (shutil.get_terminal_size().columns)))
        print(
            "WARNING: Volume of the unit cell declared in CIF ({:.2f} A^3) is different than the calculated from primitive vectors ({:.2f} A^3).\n".format(
                volume, V
            )
        )
        print("{}".format("~" * (shutil.get_terminal_size().columns)))
    ...
    print(
        "BoxLenA, BoxLenB, BoxLenC = {:.4f}, {:.4f}, {:.4f}".format(
            BoxLenA, BoxLenB, BoxLenC
        )
    )
    print(
        "BoxLenX, BoxLenY, BoxLenZ = {:.4f}, {:.4f}, {:.4f}".format(
    ...
    try:
        ...
        print(
            "\nERROR: A type error occured when opening or writting XYZ file '{0}'".format(
                fNameOut
            )
        )
        sys.exit()
...
def bfs(geom, elem, bfs_thresh):
    ...
    atomkeys = [
        (pos[0], pos[1], pos[2]) for pos in geom_floor - (np.mod(geom_floor, blocksize))
    ]
...
def center_supercell(cif_output, verbose=0):
    ...
    if len(scell_dupl) != len(clean_cell):
        print("{}".format("~" * (shutil.get_terminal_size().columns)))
        print(
            "WARNING: The supercell contains {} duplicate coordinates. Skipping duplicates.".format(
                len(scell_dupl) - len(clean_cell)
            )
        )
        ...
        scell_geom_dupl = np.loadtxt(
            cif_output, skiprows=2, usecols=(1, 2, 3), dtype=np.float64
        )
        ...
        print(
            "         The number of unique coordinates in the supercell is now: {}\n".format(
                len(clean_cell)
            )
        )
        print("{}".format("~" * (shutil.get_terminal_size().columns)))
...
def supercell2monomers(cif_output, r_cut_monomer, bfs_thresh, verbose=1):
    ...
    scell_geom_max_coords, scell_geom, scell_elem = center_supercell(
        cif_output, verbose
    )
    ...
    if (r_cut_monomer / qcel.constants.bohr2angstroms) > np.min(scell_geom_max_coords):
        print(
            ...
            % (
                r_cut_monomer,
                np.min(scell_geom_max_coords) * qcel.constants.bohr2angstroms,
            )
        )
        print(
            "       Please increase the dimensions of the supercell to at least twice r_cut_monomer or reduce the lenght of the cutoff.\n"
        )
        # sys.exit()
    ...
    if verbose >= 2:
        print(
            "\nThe BFS algorithm found {} monomers in the supercell in {:.2f} s".format(
                total_number_of_monomers, bfs_stop_time
            )
        )
...
def create_nmer(nmers, ref_monomer, other_monomers, verbose=1):
    ...
    for a in nm_new_monomers:
        ...
        for b in nm_new_monomers:
            ...
            distm, r_min = distance_matrix(
                nmers[a_name]["coords"], nmers[b_name]["coords"]
            )
            nm_new["min_monomer_separations"].append(r_min)
            nm_new["com_monomer_separations"].append(
                np.linalg.norm(nmers[a_name]["com"] - nmers[b_name]["com"])
            )
    ...
    nm_new_name = (
        str(len(nm_new_monomers)) + "mer-" + "+".join(map(str, nm_new_monomers))
    )
...
def nmername_to_int(name):
    ...
    monomer_weights = [
        max_monomers**monomer_weight for monomer_weight in monomer_weights
    ]

    nmer_hash = np.sum(
        [monomer_weights[i] * monomer_inds[i] for i in range(len(monomer_inds))]
    )
...
def build_nmer(
    ...
    total_monomers,
    nmer_type,
    nmer_separation_cutoff,
    coms_separation_cutoff,
    uniq_filter,
    verbose=1,
):
    ...
    if num_monomers > 2:
        for ref_monomer_idx in range(num_ref_monomers):
            ...
            for other_monomers in range(ref_monomer_idx + 1, total_monomers):
                ...
                new_nmer_name, new_nmer = create_nmer(
                    nmers, ref_monomer, (other_monomers,), verbose
                )
                ...
                if new_nmer["max_mon_sep"] > (
                    nmer_separation_cutoff / qcel.constants.bohr2angstroms
                ):
                    if verbose >= 2:
                        ...
                        print(
                            "Monomer {} discarded from {} list in {:.2f} s: Maximum separation between closest atoms of different monomers is {:3.2f} A, longer than cutoff {:3.2f} A.".format(
                                ...
                                nmer_type,
                                nmer_stop_time,
                                new_nmer["max_mon_sep"] * qcel.constants.bohr2angstroms,
                                nmer_separation_cutoff,
                            )
                        )
                ...
                elif new_nmer["max_com_sep"] > (
                    coms_separation_cutoff / qcel.constants.bohr2angstroms
                ):
                    if verbose >= 2:
                        # Stop N-mer building timer.
                        nmer_stop_time = time.time() - nmer_start_time
                        print(
                            "{} discarded in {:.2f} s: Maximum separation between closest COMs of different monomers is {:3.2f} A, longer than cutoff {:3.2f} A.".format(
                                new_nmer_name,
                                nmer_stop_time,
                                new_nmer["max_com_sep"] * qcel.constants.bohr2angstroms,
                                coms_separation_cutoff,
                            )
                        )
        ...
        for ref_monomer_idx in range(num_ref_monomers):
            nmer_monomer_list[ref_monomer_idx] = range(
                ref_monomer_idx + 1, total_monomers
            )
    ...
    for ref_monomer_idx in range(num_ref_monomers):
        ...
        for other_monomers in itertools.combinations(
            nmer_monomer_list[ref_monomer_idx], num_monomers - 1
        ):
            # Start N-mer building timer.
            nmer_start_time = time.time()
            ...
            # Third, employ more sophisticated filters: NRE and ChSEV or Dreamaligner.
                ...
                nre_tolerance = 1e-8
                close_nmer_keys = close_nmers(
                    new_nmers_nre_sorted, new_nmer["nre"], nre_tolerance
                )
                ...
                for kexisting in close_nmer_keys:
                    ...
                    # Fifth, if the NRE is small, proceed to next filters.
                        ...
                        if chemical_space == True:
                            chsev_diff = np.linalg.norm(
                                existing["chsev"] - new_nmer["chsev"]
                            )
                            chsev_filter_ran = True

                            if chsev_diff < 1.0e-8:
                                ...
                                if verbose >= 2:
                                    ...
                                    print(
                                        "{} discarded in {:.2f} s: Replica of {}. NRE difference is {:.1e} and ChSEV difference is {:.1e}.".format(
                                            new_nmer_name,
                                            nmer_stop_time,
                                            kexisting,
                                            nre_diff,
                        ...
                        if dreamliner == True:
                            ...
                            rmsd, mill = qcel.molutil.B787(
                                rgeom=existing["coords"],
                                cgeom=new_nmer["coords"],
                                runiq=existing["elem"],
                                cuniq=new_nmer["elem"],
                                run_mirror=True,
                                verbose=2,
                            ...
                            if rmsd < 1.0e-3:
                                ...
                                if verbose >= 2:
                                    ...
                                    print(
                                        "{} discarded in {:.2f} s: Replica of {}. NRE difference is {:.1e} and RMSD is {:.1e}.".format(
                                            new_nmer_name,
                                            nmer_stop_time,
                                            kexisting,
                                            nre_diff,
                ...
                if not found_duplicate:
                    ...
                    insert_nmer_nre(
                        new_nmers_nre_sorted, (new_nmer["nre"], new_nmer_name)
                    )
                    ...
                    if verbose >= 2:
                        ...
                        if nre_filter_ran == True:
                            if chsev_filter_ran == True:
                                print(
                                    "{} generated in {:.2f} s: New N-mer NRE difference is {:.1e}, lowest ChSEV difference found is {:.1e}.".format(
                                        new_nmer_name,
                                        nmer_stop_time,
                                        nre_diff,
                                        chsev_diff,
                            ...
                            if rmsd_filter_ran == True:
                                print(
                                    "{} generated in {:.2f} s: New N-mer NRE difference is {:.1e}, lowest RMSD found is {:.1e}.".format(
                            ...
                            if chsev_filter_ran == False and rmsd_filter_ran == False:
                                print(
                                    "{} generated in {:.2f} s: New N-mer NRE difference is {:.1e}.".format(
                                        new_nmer_name, nmer_stop_time, nre_diff
                                    )
                                )

                        else:
                            print(
                                "{} generated in {:.2f} s: New N-mer NRE is {:.12f}.".format(
    ...
    if verbose >= 2:
        ...
        print(
            "\n{} unique {} were found and generated in {:.1f} s.".format(
                counter_new_nmers, nmer_type, build_nmers_stop_time
            )
        )

    if verbose >= 2:
        print(
            "\n{} {} did not meet the atomic separation cutoff and were discarded.".format(
                counter_dscrd_sep, nmer_type
            )
        )
        print(
            "{} {} did not meet the center of mass separation cutoff and were discarded.".format(
                counter_dscrd_com, nmer_type
            )
        )
        print(
            "{} {} were duplicates of another dimer and were discarded.".format(
                counter_dscrd_rep, nmer_type
            )
        )
...
def monomer2makefp(cif_output, nmer, verbose=0):
    ...
    for at in range(nmer["coords"].shape[0]):
        ...
        makefp_input += " {:6} {:4.1f} {:16.8f} {:16.8f} {:16.8f} \n".format(
            nmer["elem"][at] + str(at + 1),
            qcel.periodictable.to_Z(nmer["elem"][at]),
            nmer["coords"][at][0],
            nmer["coords"][at][1],
            nmer["coords"][at][2],
        )
...
def nmer2psithon(
    ...
    mymol = (
        keynmer.replace("2mer", "Dimer")
        .replace("3mer", "Trimer")
        .replace("4mer", "Tetramer")
        .replace("5mer", "Pentamer")
        .replace("-", "_")
        .replace("+", "_")
    )
    psithon_input += "\nmolecule {} {{\n".format(mymol)
...
def nmer2qcmanybody(
    ...
    if "test" not in cle_run_type:
        ...
        varstring = "{}-CORRECTED INTERACTION ENERGY THROUGH {}-BODY".format(
            psi4_bsse.upper(), str(len(nmer["monomers"]))
        )
        ...
        if len(nmer["monomers"]) > 2:
            varstring = "{}-CORRECTED INTERACTION ENERGY THROUGH {}-BODY".format(
                psi4_bsse.upper(), str(len(nmer["monomers"]) - 1)
            )
...
def nmer2libefpmbe(cif_output, nmers, keynmer, nmer, rminseps, rcomseps, verbose=0):
    ...
    frgname = cif_output.split(".")[
        0
    ]  # TODO: Implement input keyword for fragment names.
    ...
    for i in range(len(nmer["monomers"])):
        ...
        libefpmbe_input += (
            "# WARNING: This part of the CrystaLattE code is designed to work with\n"
        )
        libefpmbe_input += (
            "#          Prof. Lyudmila V. Slipchenko's fork of LibEFP 1.5.0 which can\n"
        )
        libefpmbe_input += "#          be found at:\n"
        libefpmbe_input += "#          https://github.com/libefp2/libefp.git\n\n"

        libefpmbe_input += (
            "# WARNING: This implementation should be limited to the computation of\n"
        )
        libefpmbe_input += (
            "#          dimers and trimers. This is due to the limitation introduced\n"
        )
        libefpmbe_input += (
            "#          by the usage of LibEFP, which at the moment is not able to\n"
        )
        libefpmbe_input += (
            "#          decompose the many-body energy into lower-order energies.\n\n"
        )

        libefpmbe_input += (
            "# WARNING: Name of the fragment are assigned based on the CIF name.\n"
        )
        libefpmbe_input += (
            "#          Check that the name of the fragments corresponds to the\n"
        )
        libefpmbe_input += (
            "#          fragment name field in the .efp potential file.\n\n"
        )
        ...
        for at in range(nmer["coords"].shape[0]):
            ...
            if line_idx < 3:
                libefpmbe_input += "{:16.8f} {:16.8f} {:16.8f} \n".format(
                    nmer["coords"][at][0], nmer["coords"][at][1], nmer["coords"][at][2]
                )
    ...
    **kwargs,
    ...
    for keynmer in nmer_keys:
        ...
        # Produce LibEFP inputs with non-embedded N-mers as fragments.
            ...
            nmer2libefpmbe(
                cif_output, nmers, keynmer, nmer, rminseps, rcomseps, verbose
            )
        ...
        nmer["contrib"] = (
            nmer["nambe"] * nmer["replicas"] / float(len(nmer["monomers"]))
        )
        ...
        if (
            ("test" in cle_run_type)
            or ("psithon" in cle_run_type)
            or ("libefpmbe" in cle_run_type)
        ):
            nmer_result = (
                "{:26} | {:>12} | {:>4} | {:>12} | {:>13} | {:12.6e} | {}".format(
                    keynmer,
                    "Not Computed",
                    nmer["replicas"],
                    "Not Computed",
            ...
            nmer_result = "{:26} | {:>12.8f} | {:>4} | {:>12.8f} | {:>13.8f} | {:12.6e} | {}".format(
                ...
                * qcel.constants.cal2J,
                nmer["priority_cutoff"],
                rminseps,
            )
            output_data["keynmer"].append(keynmer)
            output_data["nambe"].append(
                nmer["nambe"] * qcel.constants.hartree2kcalmol * qcel.constants.cal2J
            )
            output_data["replicas"].append(nmer["replicas"])
            output_data["contrib"].append(
                nmer["contrib"] * qcel.constants.hartree2kcalmol * qcel.constants.cal2J
            )
            output_data["crystal_lattice_energy"].append(
                crystal_lattice_energy
                * qcel.constants.hartree2kcalmol
                * qcel.constants.cal2J
            )
            output_data["priority_cutoff"].append(nmer["priority_cutoff"])
            output_data["min_separations"].append(rminseps)
        ...
        if verbose >= 2:
            if "psi4api" in cle_run_type:
                print(
                    "{} elapsed {:.2f} s processing on {} threads. Cumulative Lattice Energy = {:9.8f} kJ/mol".format(
                        keynmer,
                        energies_wallclock,
                        cpus,
                        crystal_lattice_energy
...
def print_results(results, crystal_lattice_energy, verbose=0):
    ...
    if verbose >= 1:
        ...
        print(
            "---------------------------+--------------+------+--------------+---------------+--------------+----------------{}".format(
                "-" * (shutil.get_terminal_size().columns - 112)
            )
        )
        print(
        ...
        print(
            "Crystal Lattice Energy (kJ/mol)   = {:9.8f}".format(
                crystal_lattice_energy
                * qcel.constants.hartree2kcalmol
                * qcel.constants.cal2J
            )
        ...
        print(
            "Crystal Lattice Energy (kcal/mol) = {:9.8f}\n".format(
                crystal_lattice_energy * qcel.constants.hartree2kcalmol
            )
        )
...
def main(
    ...
    **kwargs,
    ...
    r_cut_monomer = cif_main(
        cif_input, cif_output, cif_a, cif_b, cif_c, r_cut_monomer, nmer_cutoff, True
    )
    ...
    crystal_lattice_energy, results, output_data = cle_manager(
        cif_output,
        nmers,
        cle_run_type,
        method,
